...
const ContactList = ({ toggleDisplay }) => {
     ...
     useEffect(() => {
          chatList?.chatList?.forEach((people, _) => {
               setPeopleList((prev) => {
                                   ...
                                   (
                                        ...
                                        <div className="overflow-y-auto w-full divide-y divide-gray-400/20">
                                             {chatList?.chatList?.filter(person => person?._id !== loggedUser).map((person, index) => (
                                                  <Link to={getChatLink(loggedUser, person?._id)} style={{ textDecoration: 'none' }} key={index} onClick={() => handleItemClick(loggedUser, person._id, person?.secondPerson[0]?.profilePhoto)} className="hover:opacity-50 cursor-pointer relative flex items-center gap-4 p-2 duration-200 active:opacity-60">
                                                       <div className='flex items-center place-content-center w-[98%] gap-x-4'>
                                                            <div className="relative w-14 h-14 shrink-0">
                                                                 {commeterProfilePhoto === undefined ? (
                                                                      <div className="capitalize text-white text-center flex place-content-center items-center w-10 h-10 md:w-14 md:h-14 shadow-[1px_1px_1px_1px_#f2f2f2] rounded-full text-xl md:text-3xl"
                                                                           style={{ backgroundColor: colorArray[Math.floor(Math.random() * colorArray.length)] }}>
                                                                           {person?.firstName?.[0] || "U"}
                                                                      </div>
                                                                 ) : (
                                                                      commeterProfilePhoto?.some(photo => photo?.userId === person?._id && photo?.profilePhoto) ? (
                                                                           commeterProfilePhoto.map((photo, index) => (
                                                                                photo?.userId === person?._id && (
                                                                                     <img key={index + person?._id} src={photo?.profilePhoto} onError={(e) => e.target.src = noProfilePicture} alt="Profile" className="text-white w-10 h-10 md:w-14 md:h-14 shadow-[1px_1px_1px_1px_#f2f2f2] border rounded-full object-cover" />)
                                                                           ))
                                                                      ) : (
                                                            ...
                                                            <div className="flex-1">
                                                                 ...
                                                                 <div className="text-black font-medium overflow-hidden text-ellipsis text-[13px] -mt-1 whitespace-nowrap">{person?.userName}</div>
                                                            </div>
                                                       </div>
                                                       {person?.unreadCount > 0 && (
                                                            <div className='scale-75 rounded-full px-[10px] py-0.5 h-fit w-fit text-white font-bold bg-[#5a70b7]'>{person?.unreadCount > 10 ? person?.unreadCount + '+' : person?.unreadCount}</div>
                                                       )}
                                                  </Link>
                                             ))}
