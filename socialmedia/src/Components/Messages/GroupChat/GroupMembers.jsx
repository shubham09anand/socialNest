...
const GroupMembers = ({ display, groupAdmin, ownerId }) => {
     ...
     const makeMoreAdmin = async ({ groupId, newAdminId }) => {
     ...
     return (
          ...
          <>
               <div className={`overflow-y-scroll border-black h-[calc(100%-40px)] sm:h-[calc(100%-160px)] border-l bg-white fixed right-0 top-16 lg:top-28 z-20 sm:z-0 pb-16 transition-all duration-[1s] ${display === 2 ? " w-full sm:w-2/3 md:w-2/4 lg:w-[40%] xl:w-[27%]" : "w-0"}`}>
                    {warning && <div className='flex-shrink-0 text-lg text-center font-thin bg-red-600 text-white w-full absolute py-2'>Something Went Wrong</div>}

                    <div className='flex-shrink-0 text-2xl font-semibold my-2 mb-1 pl-2'>Group Members</div>
                    ...
                    {isLoading ? (
                         ...
                         <ul className="w-full p-2">
                              {data?.data?.members?.map((items, index) => (
                                   <li key={index} className={`relative flex items-center py-2 space-x-4 ${option === null || option === index ? 'opacity-100' : 'opacity-30'}`}>
                                        <div className='h-fit w-fit border-2 border-black rounded-full '>
                                             ...
                                             option === index && (
                                                  <div className='bg-white select-none absolute right-10 sm:right-12 top-2 w-fit h-fit divide-y divide-gray-700 px-2 border shadow-[2px_2px_2px_black] rounded-lg'>
                                                       {(loggedUserId === ownerId) && (
                                                            <button disabled={isRemoving} onClick={() => handleRemoveMember({ groupId: groupId, userId: items._id })} className={`flex space-x-2 sm:space-x-4 py-1 hover:opacity-75 active:opacity-50 items-center ${isRemoving ? 'cursor-wait opacity-50' : 'cursor-pointer opacity-100'}`}>
                    ...
                    {
                         isError && (
                              <ServerError width={1 / 2} height={40} />
                         )
                    }
               </div>
          </>
